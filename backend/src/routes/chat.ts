...
async function getStreamSlowMode (streamId: string): Promise<number> {
  ...
  try {
    const parsed = typeof res.rows[0].settings === 'string' ? JSON.parse(String(res.rows[0].settings)) : {}
  ...
  app.get('/chat/:streamId', { websocket: true }, (connection, req) => {
    ...
    void (async () => {
      ...
      for (const raw of pendingMessages) {
        await processMessage(raw, client, streamId, room ?? new Set(), connection)
      }
    ...
    connection.socket.on('message', (raw: Buffer) => {
      ...
      void processMessage(raw, client, streamId, room ?? new Set(), connection)
    })
