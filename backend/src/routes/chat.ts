...
async function getStreamSlowMode (streamId: string): Promise<number> {
  ...
  try {
    const parsed = typeof res.rows[0].settings === 'string' ? JSON.parse(res.rows[0].settings as string) : (res.rows[0].settings as Record<string, unknown> ?? {})
    return typeof (parsed as Record<string, unknown>).chat_slow_mode === 'number' ? (parsed as Record<string, unknown>).chat_slow_mode as number : 0
  } catch {
    return 0
  }
}
...
export async function registerChatRoutes (app: FastifyInstance): Promise<void> {
  ...
  app.get<{ Params: ChatParams }>('/chat/:streamId', { websocket: true }, (connection, req) => {
