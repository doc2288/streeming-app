...
async function getStreamSlowMode (streamId: string): Promise<number> {
  ...
  try {
    const parsed = (typeof res.rows[0].settings === 'string' ? JSON.parse(res.rows[0].settings as string) : {}) as Record<string, unknown>
    return typeof parsed.chat_slow_mode === 'number' ? Number(parsed.chat_slow_mode) : 0
  } catch {
    return 0
  }
}
