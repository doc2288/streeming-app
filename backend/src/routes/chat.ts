...
async function getStreamSlowMode (streamId: string): Promise<number> {
  ...
  try {
    const parsed = typeof res.rows[0].settings === 'string' ? JSON.parse(String(res.rows[0].settings)) : res.rows[0].settings
    if (parsed !== null && typeof parsed === 'object' && 'chat_slow_mode' in parsed) {
