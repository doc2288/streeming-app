...
export async function registerStreamRoutes (app: FastifyInstance): Promise<void> {
  app.get('/streams', async (request: FastifyRequest) => {
    ...
    const limit = Math.min(Math.max(Number.isNaN(parseInt(query.limit ?? '50', 10)) || parseInt(query.limit ?? '50', 10) === 0 ? 50 : parseInt(query.limit ?? '50', 10), 1), 100)
    const offset = Math.max(Number.isNaN(parseInt(query.offset ?? '0', 10)) ? 0 : parseInt(query.offset ?? '0', 10), 0)
    ...
    const streams = res.rows.map((s: Record<string, unknown>) => {
      ...
      return {
        ...
        tags: typeof s.tags === 'string' && s.tags !== '' ? String(s.tags).split(',') : [],
        settings,
        status: s.status,
