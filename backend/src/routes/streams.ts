...
export async function registerStreamRoutes (app: FastifyInstance): Promise<void> {
  app.get('/streams', async (request: FastifyRequest) => {
    ...
    const limit = Math.min(Math.max(parseInt(query.limit ?? '50', 10) !== 0 && !isNaN(parseInt(query.limit ?? '50', 10)) ? parseInt(query.limit ?? '50', 10) : 50, 1), 100)
    const offset = Math.max(parseInt(query.offset ?? '0', 10) !== 0 && !isNaN(parseInt(query.offset ?? '0', 10)) ? parseInt(query.offset ?? '0', 10) : 0, 0)
    ...
    const streams = res.rows.map((s: Record<string, unknown>) => {
      ...
      return {
        ...
        stream_key: s.user_id === userId ? (s.stream_key as string) : null,
        user_id: s.user_id,
