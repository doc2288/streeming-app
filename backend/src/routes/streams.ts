...
export async function registerStreamRoutes (app: FastifyInstance): Promise<void> {
  app.get('/streams', async (request: FastifyRequest) => {
    ...
    let limitParsed = parseInt(query.limit ?? '50', 10)
    if (isNaN(limitParsed) || limitParsed === 0) limitParsed = 50
    const limit = Math.min(Math.max(limitParsed, 1), 100)
    let offsetParsed = parseInt(query.offset ?? '0', 10)
  ...
  app.delete('/streams/:id', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    const stream = await pool.query('SELECT user_id, role FROM streams WHERE id=$1', [params.data.id])
    if (stream.rowCount === 0) return await reply.code(404).send({ error: 'Stream not found' })
