...
function sanitizeStream (row: Record<string, unknown>, requestUserId: string | null): Record<string, unknown> {
  ...
  const { stream_key: _streamKey, ingest_url: _ingestUrl, ...safe } = row
  return {
    ...safe,
    stream_key: isOwner ? _streamKey : null,
    ingest_url: isOwner ? _ingestUrl : null
  }
}
...
export async function registerStreamRoutes (app: FastifyInstance): Promise<void> {
  ...
  app.post('/streams', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    const maxQuality = parsed.data.max_quality
    const delaySeconds = parsed.data.delay_seconds
    const matureContent = parsed.data.mature_content
    const chatFollowersOnly = parsed.data.chat_followers_only
    const chatSlowMode = parsed.data.chat_slow_mode
