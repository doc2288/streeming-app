...
export async function registerStreamRoutes (app: FastifyInstance): Promise<void> {
  app.get('/streams', async (request: FastifyRequest) => {
    ...
    const limit = Math.min(Math.max(parseInt(query.limit ?? '50', 10) === 0 || Number.isNaN(parseInt(query.limit ?? '50', 10)) ? 50 : parseInt(query.limit ?? '50', 10), 1), 100)
    const offset = Math.max(parseInt(query.offset ?? '0', 10) === 0 || Number.isNaN(parseInt(query.offset ?? '0', 10)) ? 0 : parseInt(query.offset ?? '0', 10), 0)
