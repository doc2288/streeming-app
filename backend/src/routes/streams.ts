...
export async function registerStreamRoutes (app: FastifyInstance): Promise<void> {
  app.get('/streams', async (request: FastifyRequest) => {
    ...
    const limitVal = parseInt(query.limit ?? '50', 10)
    const limit = Math.min(Math.max(!isNaN(limitVal) && limitVal > 0 ? limitVal : 50, 1), 100)
    const offsetVal = parseInt(query.offset ?? '0', 10)
    const offset = Math.max(!isNaN(offsetVal) && offsetVal > 0 ? offsetVal : 0, 0)
