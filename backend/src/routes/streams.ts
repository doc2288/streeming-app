...
function sanitizeStream (row: Record<string, any>, requestUserId: string | null): Record<string, unknown> {
...
export async function registerStreamRoutes (app: FastifyInstance): Promise<void> {
  app.get('/streams', async (request: FastifyRequest) => {
    ...
    const streams = res.rows.map((s: Record<string, any>) => {
  ...
  app.post('/streams', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    return { stream: sanitizeStream(updated.rows[0] as Record<string, any>, request.user.sub) }
  })
