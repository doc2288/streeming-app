...
export async function registerStreamRoutes (app: FastifyInstance): Promise<void> {
  app.get('/streams', async (request: FastifyRequest) => {
    ...
    const pLimit = parseInt(query.limit ?? '50', 10)
    const limit = Math.min(Math.max(Number.isNaN(pLimit) ? 50 : pLimit === 0 ? 50 : pLimit, 1), 100)
    const pOffset = parseInt(query.offset ?? '0', 10)
