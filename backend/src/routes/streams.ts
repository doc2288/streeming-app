...
export async function registerStreamRoutes (app: FastifyInstance): Promise<void> {
  app.get('/streams', async (request: FastifyRequest) => {
    ...
    const limitParsed = parseInt(query.limit ?? '50', 10); const limit = Math.min(Math.max(Number.isNaN(limitParsed) ? 50 : limitParsed, 1), 100)
    const offsetParsed = parseInt(query.offset ?? '0', 10); const offset = Math.max(Number.isNaN(offsetParsed) ? 0 : offsetParsed, 0)
