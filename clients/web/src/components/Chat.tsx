    ...
    <div className="chat-panel">
      <div className="chat-header">
        ...
        <div className="chat-header-actions">
          <button className="chat-header-btn" onClick={() => { setShowRules(!showRules) }} title={t('chatRules')} aria-label={t('chatRules')} aria-expanded={showRules}>
      ...
      <div className="chat-input-area">
        <button className="chat-emoji-toggle" onClick={() => { setShowEmoji(!showEmoji) }} title="Emoji" aria-label="Toggle emoji picker" aria-expanded={showEmoji}>
        ...
        <input ref={inputRef} value={text} onChange={(e) => { setText(e.target.value) }} onKeyDown={(e) => { if (e.key === 'Enter') send() }} placeholder={connected ? t('chatPlaceholder') : t('connecting')} aria-label="Chat input" maxLength={500} disabled={!connected} />
        <button onClick={send} disabled={!connected || text.trim().length === 0} className="chat-send" aria-label="Send message">
