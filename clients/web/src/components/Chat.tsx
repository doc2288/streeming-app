  ...
  return (
    <div className="chat-panel">
            ...
            <div key={i} className={`chat-line ${m.type === 'action' ? 'chat-action' : ''}`} onMouseEnter={() => { setHoveredMsg(i) }} onMouseLeave={() => { setHoveredMsg(null) }}>
              {hoveredMsg === i && (
                <div className="chat-line-actions">
                  <button className="chat-react-btn" onClick={() => { replyTo(name) }} title={t('replyBtn')} aria-label={t('replyBtn')}>↩</button>
      ...
      <div className="chat-input-area">
        <button className="chat-emoji-toggle" onClick={() => { setShowEmoji(!showEmoji) }} title={t('emojiBtn')} aria-label={t('emojiBtn')}>
