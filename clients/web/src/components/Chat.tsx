            ...
            <div key={i} className={`chat-line ${m.type === 'action' ? 'chat-action' : ''}`} onMouseEnter={() => { setHoveredMsg(i) }} onMouseLeave={() => { setHoveredMsg(null) }}>
              {hoveredMsg === i && (
                <div className="chat-line-actions">
                  <button className="chat-react-btn" onClick={() => { replyTo(name) }} title={t('replyAction')} aria-label={t('replyAction')}>↩</button>
                  {QUICK_REACTIONS.map(e => <button key={e} className="chat-react-btn" onClick={() => { addReaction(i, e) }} title={t('reactAction')} aria-label={t('reactAction')}>{e}</button>)}
                </div>
              )}
