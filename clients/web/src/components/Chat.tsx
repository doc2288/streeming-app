  ...
  return (
    <div className="chat-panel">
            ...
            <div key={i} className={`chat-line ${m.type === 'action' ? 'chat-action' : ''}`} onMouseEnter={() => { setHoveredMsg(i) }} onMouseLeave={() => { setHoveredMsg(null) }}>
              {hoveredMsg === i && (
                <div className="chat-line-actions">
                  <button className="chat-react-btn" onClick={() => { replyTo(name) }} title={t('replyTo')} aria-label={t('replyTo')}>↩</button>
      ...
      <div className="chat-input-area">
        <button className="chat-emoji-toggle" onClick={() => { setShowEmoji(!showEmoji) }} title={t('emojiToggle')} aria-label={t('emojiToggle')}>
        ...
        <button onClick={send} disabled={!connected || text.trim().length === 0} className="chat-send" title={t('sendTitle')} aria-label={t('sendTitle')}>
