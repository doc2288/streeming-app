    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => { e.stopPropagation() }}>
        ...
        <form className="modal-form" onSubmit={(e) => { void handleSubmit(e) }}>
          <div className="form-group"><label htmlFor="auth-email">{t('email')}</label><input id="auth-email" type="email" placeholder="your@email.com" value={email} onChange={(e) => { setEmail(e.target.value) }} autoFocus aria-invalid={error != null} /></div>
          <div className="form-group"><label htmlFor="auth-password">{t('password')}</label><input id="auth-password" type="password" placeholder={t('passwordMin')} value={password} onChange={(e) => { setPassword(e.target.value) }} aria-invalid={error != null} /></div>
          {error != null && <div className="form-error" role="alert" aria-live="assertive">{error}</div>}
