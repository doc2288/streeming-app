...
export function AuthModal ({ onClose, onSuccess }: Props): JSX.Element {
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      const msg = err.response?.data?.error as unknown
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => { e.stopPropagation() }}>
        <button className="modal-close" onClick={onClose} aria-label="Close modal"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 6L6 18M6 6l12 12" /></svg></button>
        ...
        <form className="modal-form" onSubmit={(e) => { void handleSubmit(e) }}>
          ...
          <div className="form-group"><label htmlFor="password">{t('password')}</label><input id="password" type="password" placeholder={t('passwordMin')} value={password} onChange={(e) => { setPassword(e.target.value) }} required /></div>
