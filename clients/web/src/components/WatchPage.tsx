        ...
        <div className="watch-info">
          ...
          <div className="watch-actions">
            <div className="quality-picker" style={{ position: 'relative' }}>
              <button className="btn-icon" onClick={() => { setShowQuality(!showQuality) }} title={t('viewers_quality')} aria-label={t('viewers_quality')}>
            ...
            {isOwner && (
              <>
                {stream.status !== 'live'
                  ? <button className="btn-go-live" onClick={() => { void handleStart() }} aria-label={t('goLive')}><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z" /></svg>{t('goLive')}</button>
                  : <button className="btn-stop" onClick={() => { void handleStop() }} aria-label={t('stopStream')}><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="6" width="12" height="12" rx="1" /></svg>{t('stopStream')}</button>
                }
                <button className="btn-icon btn-danger-icon" onClick={() => { void handleDelete() }} title="Видалити стрім" aria-label="Видалити стрім">
            ...
            <button className="btn-icon" onClick={onBack} title={t('back')} aria-label={t('back')}>
        ...
        {isOwner && stream.ingest_url != null && (
          <div className="watch-ingest">
            <div className="ingest-row">
              ...
              <code>{obsServer}</code>
              <button className="btn-copy" onClick={() => { copyToClipboard(obsServer) }} title="Копіювати" aria-label="Копіювати сервер">
            ...
            <div className="ingest-row">
              ...
              <button className="btn-copy" onClick={() => { copyToClipboard(stream.stream_key ?? null) }} title="Копіювати" aria-label="Копіювати ключ">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <rect x="9" y="9" width="13" height="13" rx="2" /><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1" />
                </svg>
              </button>
