...
export function TopBar ({ user, onLogin, onLogout, onSearch, onNavigateHome, sidebarOpen, onToggleSidebar, searchValue }: Props): JSX.Element {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent): void => {
      if (dropRef.current != null && !dropRef.current.contains(e.target as Node)) setDropdownOpen(false)
      if (langRef.current != null && !langRef.current.contains(e.target as Node)) setLangOpen(false)
    }
    document.addEventListener('mousedown', handler)
    return () => { document.removeEventListener('mousedown', handler) }
  }, [])

  const handleSubmit = (e: React.FormEvent): void => { e.preventDefault(); onSearch(query.trim()) }
    ...
    <nav className="topbar">
      ...
      <div className="topbar-right">
        <div className="lang-picker" ref={langRef}>
          <button className="lang-btn" onClick={() => { setLangOpen(!langOpen) }} title="Language">
            ...
            <div className="lang-dropdown">
              {(['ua', 'en', 'ru'] as Lang[]).map(l => (
                <button key={l} className={`lang-option ${l === lang ? 'active' : ''}`} onClick={() => { setLang(l); setLangOpen(false) }}>
                  <span>{FLAGS[l]}</span>
                  <span>{l === 'ua' ? 'Українська' : l === 'en' ? 'English' : 'Русский'}</span>
                </button>
              ))}
            </div>
          )}
        ...
        {user != null ? (
          <div className="topbar-user" ref={dropRef}>
            <button className="avatar" onClick={() => { setDropdownOpen(!dropdownOpen) }}>{user.email[0].toUpperCase()}</button>
            {dropdownOpen && (
