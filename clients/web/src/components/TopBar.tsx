  ...
  return (
    <nav className="topbar">
      <div className="topbar-left">
        <button className="topbar-hamburger" onClick={onToggleSidebar} aria-label={t('menu')}>
      ...
      <form className="topbar-search" onSubmit={(e) => { e.preventDefault(); onSearch(query.trim()) }}>
        ...
        {query.length > 0 && (
          <button type="button" className="search-clear" onClick={() => { setQuery(''); onSearch('') }} aria-label={t('close')}>×</button>
        )}
        <button type="submit" className="search-btn" aria-label={t('searchBtn')}>
      ...
      <div className="topbar-right">
        {user != null
          ...
          <div className="topbar-user" ref={menuRef}>
            <button className="avatar" onClick={() => { setMenuOpen(!menuOpen) }} aria-label={t('menu')}>{user.email[0].toUpperCase()}</button>
            {menuOpen && (
