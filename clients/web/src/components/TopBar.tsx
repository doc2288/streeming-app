  ...
  return (
    <nav className="topbar">
      <div className="topbar-left">
        <button className="topbar-hamburger" onClick={onToggleSidebar} aria-label={t('menuToggle')} title={t('menuToggle')}>
      ...
      <form className="topbar-search" onSubmit={(e) => { e.preventDefault(); onSearch(query.trim()) }}>
        ...
        {query.length > 0 && (
          <button type="button" className="search-clear" aria-label={t('clearSearch')} title={t('clearSearch')} onClick={() => { setQuery(''); onSearch('') }}>×</button>
        )}
        <button type="submit" className="search-btn" aria-label={t('searchBtn')} title={t('searchBtn')}>
