  ...
  return (
    <nav className="topbar">
      ...
      <form className="topbar-search" onSubmit={(e) => { e.preventDefault(); onSearch(query.trim()) }}>
        ...
        {query.length > 0 && (
          <button type="button" className="search-clear" onClick={() => { setQuery(''); onSearch('') }} aria-label={t('clear')} title={t('clear')}>×</button>
        )}
        <button type="submit" className="search-btn" aria-label={t('searchAction')} title={t('searchAction')}>
