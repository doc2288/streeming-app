...
export default function App (): JSX.Element {
  ...
  return (
    <div className="app">
      ...
      {showCreate && (
        <div className="modal-overlay" onClick={() => { setShowCreate(false) }}>
          <div className="modal modal-lg" onClick={(e) => { e.stopPropagation() }}>
            <button className="modal-close" aria-label="Close modal" onClick={() => { setShowCreate(false) }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 6L6 18M6 6l12 12" /></svg></button>
                ...
                <div className="thumb-upload-area">
                  {thumbPreview != null ? (
                    <div className="thumb-preview">
                      ...
                      <button type="button" className="thumb-remove" aria-label="Remove thumbnail" onClick={() => { setNewThumb(null); setThumbPreview(null) }}>×</button>
                    </div>
                  ) : (
      ...
      {toast != null && <div className={`toast ${toast.type}`}><span>{toast.text}</span><button className="toast-x" aria-label="Dismiss notification" onClick={() => { setToast(null) }}>×</button></div>}
    </div>
  )
}
