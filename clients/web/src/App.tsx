...
import { useI18n, CATEGORIES, getCategoryKey, STREAM_LANGUAGES, type Category, type Lang } from './i18n'
...
export default function App (): JSX.Element {
  ...
  const fetchStreams = useCallback(async () => {
    try { const r = await api.get('/streams'); setStreams(Array.isArray((r.data as { streams: Stream[] }).streams) ? (r.data as { streams: Stream[] }).streams : []) } catch {}
  }, [])
  ...
  const restoreSession = useCallback(async () => {
    ...
    try {
      ...
      setUser((res.data as { user: UserInfo }).user)
    } catch (error) {
