...
export default function App (): JSX.Element {
  ...
  return (
    <div className="app">
      ...
      {showCreate && (
        <div className="modal-overlay" onClick={() => { setShowCreate(false) }}>
          <div className="modal modal-lg" onClick={(e) => { e.stopPropagation() }}>
            <button className="modal-close" onClick={() => { setShowCreate(false) }} aria-label={t('hide')}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 6L6 18M6 6l12 12" /></svg></button>
                ...
                <div className="thumb-upload-area">
                  {thumbPreview != null
                    ...
                    <div className="thumb-preview">
                      ...
                      <button type="button" className="thumb-remove" onClick={() => { setNewThumb(null); setThumbPreview(null) }} aria-label={t('hide')}>×</button>
                    </div>
                      )
                    : (
      ...
      {toast != null && <div className={`toast ${toast.type}`}><span>{toast.text}</span><button className="toast-x" onClick={() => { setToast(null) }} aria-label={t('hide')}>×</button></div>}
    </div>
  )
}
